...
- This portfolio uses Vite + React and CSS files, not Tailwind. Translate layout, type, and color into existing `src/**/*.css` patterns.
...
- **Render every icon/image from its exported asset.** Never hand-write icons, never author your own icon file, never drop an icon or leave a placeholder — you don't have the real vector data, so anything you draw is wrong.
...
- **Size explicitly:** a fixed-size container (icons are usually square) with BOTH width and height set, and size the leaf image to fill it — never `auto`, which blows the image up to its intrinsic size.
